...
class DeepSeparator(nn.Module):
    ...
    def forward(self, x, indicator):
        ...
        emb_x = x#[B,512]
        learnable_atte_x = x#[B,512]

        emb_x = torch.unsqueeze(emb_x, 1)#[B,1,512]

        emb_x_1 = self.conv1_1_1(emb_x)#[B,5,512]
        ...
        emb_skip_connect_x = emb_x#[B,20,512]  记录一个残差连接的变量

        emb_x_1 = self.conv1_2_1(emb_x)#[B,5,512]
        emb_x_2 = self.conv1_2_2(emb_x)
        emb_x_3 = self.conv1_2_3(emb_x)
        emb_x_4 = self.conv1_2_4(emb_x)
        emb_x = torch.cat((emb_x_1, emb_x_2, emb_x_3, emb_x_4), dim=1)
        emb_x = torch.sigmoid(emb_x)#[B,20,512]

        emb_x_1 = self.conv1_3_1(emb_x)#[B,5,512]
        emb_x_2 = self.conv1_3_2(emb_x)
        emb_x_3 = self.conv1_3_3(emb_x)
        emb_x_4 = self.conv1_3_4(emb_x)
        emb_x = torch.cat((emb_x_1, emb_x_2, emb_x_3, emb_x_4), dim=1)#[B,20,512]
        emb_x = torch.sigmoid(emb_x)
        emb_x = emb_x + emb_skip_connect_x#[B,20,512] 第一次残差连接

        emb_x_1 = self.conv1_4_1(emb_x)#[B,5,512]
        emb_x_2 = self.conv1_4_2(emb_x)
        emb_x_3 = self.conv1_4_3(emb_x)
        emb_x_4 = self.conv1_4_4(emb_x)
        emb_x = torch.cat((emb_x_1, emb_x_2, emb_x_3, emb_x_4), dim=1)#[B,20,512]
        emb_x = self.conv1_squeeze1(emb_x)#[B,1,512]

        emb_x = torch.squeeze(emb_x, 1)#[B,512]

        '''--------------------------------------------------------------------------------'''

        learnable_atte_x = torch.unsqueeze(learnable_atte_x, 1)#[B,1,512]

        learnable_atte_x_1 = self.conv2_1_1(learnable_atte_x)#[B,5,512]
        ...
        atte_skip_connect_x = learnable_atte_x#[B,20,512]

        learnable_atte_x_1 = self.conv2_2_1(learnable_atte_x)#[B,5,512]
        ...
        learnable_atte_x = torch.sigmoid(learnable_atte_x)
        learnable_atte_x = torch.squeeze(learnable_atte_x, 1)#[B,512]

        '''--------------------------------------------------------------------------------'''

        atte_x = indicator - learnable_atte_x#[B,512]
        ...
        output = torch.unsqueeze(output, 1)#[B,1,512]
        ...
        output = torch.squeeze(output, 1)#[B,512]
